    ...
    public class InventoryPanel : MonoBehaviour
    {
        ...
        private void OnCollectAnimationCompleted(CollectAnimationCompletedEvent evt)
        {
            ...
            if (_slotMap.TryGetValue(evt.ItemType, out var slot))
            {
                ...
                var screenPos = RectTransformUtility.WorldToScreenPoint(null, slot.RectTransform.position);
                var worldPos = _cameraController.ScreenToWorldPoint(new Vector3(screenPos.x, screenPos.y, 5));

                _particleService.Play(ParticleType.UIPop, worldPos);
            }
        }
    }
}
