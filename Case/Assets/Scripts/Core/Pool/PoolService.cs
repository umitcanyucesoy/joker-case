    ...
    public class PoolService : IPoolService
    {
        ...
        private GameObject GetInternal(GameObject prefab, Vector3 position, Quaternion rotation, Transform parent)
        {
            ...
            if (_pools[key].Count > 0)
            {
                ...
                Debug.Log($"[PoolService] <color=green>REUSED</color> {prefab.name} | Pool remaining: {_pools[key].Count}");
            }
            else
            {
                instance = Object.Instantiate(prefab, position, rotation, parent);
                Debug.Log($"[PoolService] <color=yellow>NEW</color> {prefab.name} | Pool was empty");
            }
        ...
        private void ReturnInternal(GameObject prefab, GameObject instance)
        {
            ...
            instance.transform.SetParent(GetPoolParent(key));
            _pools[key].Enqueue(instance);
            Debug.Log($"[PoolService] <color=cyan>RETURNED</color> {prefab.name} | Pool count: {_pools[key].Count}");
        }
